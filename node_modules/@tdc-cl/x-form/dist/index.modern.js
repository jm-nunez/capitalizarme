...
import React__default, { createElement, useEffect, useState, createContext, useContext, useRef, useMemo } from 'react';
import Immutable, { Set } from 'immutable';
import deepExtend from 'deep-extend';
import _$1 from 'lodash/fp';
import autoBind from 'auto-bind';
import { LocalDate, DateTimeParseException, LocalTime } from 'js-joda';
...
var Option = /*#__PURE__*/function () {
  ...
  _proto.equals = function equals(other) {
    return other instanceof Option && Immutable.is(this.value, other.value) && Immutable.is(this.label, other.label);
  };
...
function useInputState(initialValue, optionsObject) {
  ...
  var _useState4 = useState(isAsync(optionsObject)),
  ...
  var _useState6 = useState(isAsync(optionsObject) ? [] : optionsObject.map(parseOption)),
...
var XFormContext = createContext({
  locale: english
});
...
function useField(field, name) {
  ...
  var _useContext = useContext(XFormContext),
...
function acceptSetOfString(data) {
  ...
  if (_.isPlainObject(data)) {
    return Set(Object.keys(data).filter(function (key) {
...
var decimal = CustomField({
  ...
  parse: function parse(input) {
    ...
    var min = new Decimal((_this$inputProps$min = this.inputProps.min) != null ? _this$inputProps$min : defaultMin);
    var max = new Decimal((_this$inputProps$max = this.inputProps.max) != null ? _this$inputProps$max : Infinity);
...
export { button as $Button, checkbox as $Checkbox, checklist as $Checklist, date as $Date, decimal as $Decimal, FieldSpec as $Field, $Form, number as $Number, password as $Password, radio as $Radio, select as $Select, $Submitter, text as $Text, textarea as $TextArea, time as $Time, CustomField, Debug, Field, FieldSpec, Form, Invalid, Option, OptionWithProps, Result, Valid, XFormContext, acceptBoolean, button, checkbox, checklist, date, decimal, english, number, optional, password, radio, removeExcessWhitespace, scrollToField, select, spanish, text, textarea, time, useForm };
